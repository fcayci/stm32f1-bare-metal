 ...
 *
 * Description:
 *   A DAC output example. Fades PA4
 *
 * DAC setup steps:
 *   1. Enable DAC clock from RCC
 *   2. Enable Alternate function clock from RCC
 *   3. Enable the relevant GPIO clock from RCC
 *     (DAC1 and DAC2 are connected to PA4 and PA5 pins respectively.)
 *   4. Set the specific DAC channel to ADC input
 *   4. Enable specific DAC channel from CR register
 *   5. Enable trigger from CR register
 *   6. Choose trigger source from CR register (sw trigger selected as example.)
 *   7. Set the value to be sent to DHR12R1 register (12-bit right justified)
 *   8. Trigger the output each time from SWTRIGR register
 *
 * Author: Furkan Cayci
 *
 *  Project Setup:
...
#define DAC_BASE        (APB1PERIPH_BASE + 0x7400) //  DAC  base address is 0x40007400
...
typedef struct
{
	...
	uint32_t SWTRIGR;   /* Address offset: 0x04 */
	uint32_t DHR12R1;   /* Address offset: 0x08 */
	uint32_t DHR12L1;   /* Address offset: 0x0C */
	uint32_t DHR8R1;    /* Address offset: 0x10 */
	uint32_t DHR12R2;   /* Address offset: 0x14 */
	uint32_t DHR12L2;   /* Address offset: 0x18 */
	uint32_t DHR8R2;    /* Address offset: 0x1C */
	uint32_t DHR12RD;   /* Address offset: 0x20 */
	uint32_t DHR12LD;   /* Address offset: 0x24 */
	uint32_t DHR8RD;    /* Address offset: 0x28 */
	uint32_t DOR1;      /* Address offset: 0x2C */
	uint32_t DOR2;      /* Address offset: 0x30 */
} DAC_type;
...
int32_t main(void)
{
	...
	RCC->APB2ENR |= (1 << 2);
	// Enable clock for DAC clock. Bit 29 in RCC APB1ENR register
	RCC->APB1ENR |= (1 << 29);
	...
	DAC->CR |= (1 << 0); // Enable DAC channel one
	DAC->CR |= (1 << 2); // Enable trigger DAC channel one
	DAC->CR |= (0b111 << 3); // Choose software trigger for DAC1 0b111 - (bits 3-5)

	// Set output to Vref * (dac_value/0xFFF)
	DAC->DHR12R1 = dac_value;
	DAC->SWTRIGR |= (1 << 0); // Trigger

	while(1)
	{
		if (0 == direction) {
			dac_value = dac_value + 0x10;
		}
		else {
			dac_value = dac_value - 0x10;
		}
