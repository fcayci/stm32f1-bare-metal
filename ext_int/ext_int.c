 ...
 *
 * Description:
 *   An external interrupt example using the buttons and LEDs.
 *   Every time either PortD Pin0 or Pin1 is pushed, PortE Pin14 toggles
 *
 * External interrupt setup steps:
 *   1. Enable AFIO clock from RCC
 *   2. Tie the pins(buttons) to EXTIx using EXTICRx
 *   3. Choose the trigger edge using RTSR or FTSR
 *   4. Mask the selected EXTIs from IMR
 *   5. Set prioirity levels from NVIC's IPR
 *   6. Enable the EXTIx interrupt from NVIC using ISER
 *
 * Author: Furkan Cayci
 *
 *  Project Setup:
 ...
 *
 * Both EXTI0 and EXTI1 point to this location.
 * We first execute what we want to do, then clear the pending bit for EXTI
 */
void button_handler(void)
{
	GPIOE->ODR ^= (1 << 14);
...
int32_t main(void)
{
	...
	// Make GPIOD Pin0 and Pin1 input
	...
	//   EXTICR1: 0b XXXX XXXX XXXX 0011
	//               pin3 pin2 pin1 pin0
	//
	//   Writing a 0b0011 to pin0 location ties PD0 to EXT0
	AFIO->EXTICR1 |= 0x0003; // Tie PD0 to EXTI0

	// EXTI1 can be configured for each GPIO module.
	//   EXTICR1: 0b XXXX XXXX 0011 XXXX
	//               pin3 pin2 pin1 pin0
	//
	//   Writing a 0b0011 to pin1 location ties PD1 to EXT1
	AFIO->EXTICR1 |= 0x0030; // Tie PD1 to EXTI1
	...
	EXTI->RTSR |= 0x00002;   // Enable rising edge trigger on EXTI1
	...
	NVIC->IPR[EXTI0_IRQn] = 0x10; // Priority level 1
	NVIC->IPR[EXTI1_IRQn] = 0x20; // Priority level 2
