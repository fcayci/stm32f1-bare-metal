 ...
 *
 * Description:
 *   A fading LED example using pwm.
 *   Toggels PortE Pin14 with fast changing intervals so that it appears
 *   as fading.
 *   PE14 is can be used as TIM1_CH4 (pinout section from STM32F107xx datasheet)
 *
 * PWM setup steps:
 *   1. Enable TIMx clock from RCC
 *   2. Enable Alternate function clock from RCC
 *   3. Enable the relevant GPIO clock from RCC
 *     (Each TIMx_CHy is connected to a GPIO pin.
 ...
 *   9. Set the operation MODE from CCMRx register (PWM mode 1) for the relevant Channel
 *   10. Enable relevant channel output and polarity from CCER
 *   11. Enable Master output from BDTR register
...
__attribute__ ((section(".vectors"))) = {
	...
	(uint32_t *) tim1_handler,      /* 0x0AC TIM1 Capture Compare            */
	0,                              /* 0x0B0 TIM2                            */
	0,                              /* 0x0B4 TIM3                            */
	0,                              /* 0x0B8 TIM4                            */
	0,                              /* 0x0BC I2C1 event                      */
...
int32_t main(void)
{
	...
	// MAP The TIM1 CH4 to PE14 pin (defaults to PA11)
